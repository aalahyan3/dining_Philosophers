...
/*   Updated: 2025/04/28 16:09:29 by aalahyan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	data->stop_sem = sem_open(STOP_SEMAPHORE, O_CREAT, 0644, 0);
	...
	data->waiter_sem = sem_open(WAITER_SEMAPHORE, O_CREAT, 0644, 1);
...
int	init_data(t_data *data)
{
	...
	O_CREAT, 0644, data->nb_philo);
	if (data->forks_sem == SEM_FAILED)
		return (0);
	sem_unlink(FORKS_SEMAPHORE);
	data->print_sem = sem_open(PRINT_SEMAPHORE, O_CREAT, 0644, 1);
